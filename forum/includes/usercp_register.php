...
if ( isset($HTTP_POST_VARS['submit']) )
{
	...
	{
		//
		// Check username from registration table.
		//
		// $sql_reg_username_check = mysql_query("SELECT username FROM users WHERE username='$username");
		// $reg_username_check = mysql_num_rows($sql_reg_username_check);
		//
		//
	...
	if ( !$error )
	{
		...
		else
		{
			...
			//
			// Insert in registration table
			//
			
			// $reg_yourpassword = md5($new_password);
			// $reg_sql = mysql_query("INSERT INTO users (first_name, last_name, email_address, username, password, info, signup_date, decrypted_password)
			// 	VALUES('', '', '$email', '$username', '$new_password', '', time(), '$reg_yourpassword')") or die (mysql_error());
			//
			//
